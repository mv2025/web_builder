...
export function StoryScrollBlock({
  ...
  useEffect(() => {
    if (isPreview) {
      const canvas = sectionRef.current?.closest(
        "[data-canvas-scroll]",
      ) as HTMLElement | null;
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-12 lg:px-16 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <div className="lg:col-span-5 text-left z-20">
              <span
                className="block text-[10px] font-mono tracking-[0.3em] text-amber-500 uppercase mb-4"
                style={{ color: "#f59e0b" }}
              >
                {subheading}
              </span>
              <h2
                ...
                style={{ whiteSpace: "pre-line", color: "#0a0a0a" }}
              >
                {heading}
              </h2>
              <p
                ...
                style={{ color: "#525252" }}
              >
                Three pillars of excellence that drive our methodology —
                ensuring sustainable growth and unparalleled performance.
              </p>
              <div
            ...
            <div className="lg:col-span-7 relative pl-0 lg:pl-16 lg:h-112.5 lg:overflow-hidden">
              ...
              >
                ...
                <div
                  className="hidden lg:block absolute left-6.75 top-56.25 h-225 w-0.75 z-0"
                  style={{ backgroundColor: "#e5e5e5" }}
                />

                {/* Animated active path line (Desktop Only) */}
                <motion.div
                  className="hidden lg:block absolute left-6.75 top-56.25 w-0.75 origin-top rounded-full z-0"
                  style={{
                    height: isDesktop ? `${scrollProgress * 900}px` : "100%",
                ...
                {promises.map((promise) => {
                  ...
                  return (
                    <div
                      key={promise.id}
                      className="lg:h-112.5 flex gap-6 sm:gap-8 items-center"
                    >
                      {/* Timeline Number Circle */}
                      <span
                        className={`w-14 h-14 rounded-full flex items-center justify-center font-extrabold text-sm shadow-md shrink-0 transition-all duration-500 z-10`}
                        style={{
                      ...
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: isActive
                            ? `0 20px 40px ${promise.glowColor}`
                            : "none",
                        }}
                      >
                        <span
                          className="block text-[10px] font-mono tracking-widest uppercase mb-2 transition-colors duration-500"
                          style={{ color: isActive ? "#f59e0b" : "#737373" }}
                        >
                          {promise.subtitle}
                        </span>
                        <h4
