...
const DEVICES: { id: Breakpoint; label: string; icon: string; width: number }[] = [
...
export function EditorToolbar() {
  const {
    project, breakpoint, setBreakpoint,
    viewport, setZoom, previewMode, setPreviewMode,
  ...
  return (
    <div style={{
      height: "var(--toolbar-height)", background: "var(--editor-toolbar)",
      ...
      <div style={{
        ...
        borderRadius: "8px", padding: "2px", border: "1px solid var(--border)",
      }}>
        {DEVICES.map((d) => (
          <button
            key={d.id}
            ...
            style={{
              ...
              background: breakpoint === d.id ? "var(--accent-muted)" : "transparent",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
        <ToolbarButton onClick={() => setZoom(viewport.zoom - 0.25)} disabled={viewport.zoom <= 0.1}>−</ToolbarButton>
        <select
          value={viewport.zoom}
          onChange={(e) => setZoom(Number(e.target.value))}
          style={{
            background: "var(--bg-hover)", border: "1px solid var(--border)",
            color: "var(--fg)", borderRadius: "6px", padding: "4px 6px", fontSize: "12px", cursor: "pointer",
          }}
        >
          {ZOOM_LEVELS.map((z) => (
        ...
        <ToolbarButton onClick={() => setZoom(viewport.zoom + 0.25)} disabled={viewport.zoom >= 3}>+</ToolbarButton>
      </div>
      ...
      <ToolbarButton
        ...
        style={{
          ...
          background: saveMsg === "Saved!" ? "rgba(34,197,94,0.15)" : "var(--accent-muted)",
      ...
      <div style={{ position: "relative" }}>
        ...
        {showExport && (
          <div style={{
            ...
            padding: "6px", minWidth: "180px", boxShadow: "var(--shadow-lg)",
          }}>
            {[
              { format: "react" as const, label: "⚛ React Project", desc: "src/ components/ pages/" },
              { format: "nextjs" as const, label: "▲ Next.js Project", desc: "app/ with TypeScript" },
              { format: "html" as const, label: "🌐 HTML + CSS + JS", desc: "Pure static files" },
            ].map((opt) => (
              <button key={opt.format} onClick={() => handleExport(opt.format)} style={{
                display: "flex", flexDirection: "column", width: "100%", padding: "10px 12px",
              ...
              >
                ...
                <span style={{ fontSize: "11px", color: "var(--fg-faint)", marginTop: "2px" }}>{opt.desc}</span>
              </button>
            ))}
          </div>
        )}
...
function ToolbarButton({
  children, onClick, disabled, title, active, style,
}: {
  ...
  return (
    <button
      ...
      style={{
        ...
        cursor: disabled ? "not-allowed" : "pointer", fontSize: "13px", lineHeight: 1,
        background: active ? "var(--accent-muted)" : "var(--bg-hover)",
        color: disabled ? "var(--fg-ghost)" : (active ? "var(--accent)" : "var(--fg-muted)"),
        outline: active ? "1px solid var(--accent-border)" : "1px solid transparent",
        border: "none",
        transition: "all 0.15s",
        ...style,
      }}
